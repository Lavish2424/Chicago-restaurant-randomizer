...
# Reset menus/filters on tab change
if action != st.session_state.last_action:
    keys_to_clear = [k for k in st.session_state if k.startswith("edit_mode_") or k.startswith("del_confirm_")]
    for k in keys_to_clear:
        del st.session_state[k]
    if action == "Random Pick (with filters)" and "last_pick" in st.session_state:
        del st.session_state.last_pick
    # Clear widget states by forcing rerun after clearing
...
if action == "View All Places":
    ...
    if not restaurants:
        ...
        # Add visited filter tab
        tab_all, tab_visited, tab_not_visited = st.tabs(["All Places", "Visited ✅", "Not Visited Yet"])

        with tab_all:
            places_to_show = restaurants
        with tab_visited:
            places_to_show = [r for r in restaurants if r.get("visited")]
        with tab_not_visited:
            places_to_show = [r for r in restaurants if not r.get("visited")]

        col_search, col_sort = st.columns([5, 3])
        with col_search:
            search_term = st.text_input("🔍 Search name, cuisine, neighborhood, address", key="search_input")
        with col_sort:
            sort_option = st.selectbox("Sort by", ["A-Z (Name)", "Latest Added", "Favorites First"], key="sort_option")

        filtered = places_to_show.copy()
        if search_term:
                        ...
                        for i, url in enumerate(r["photos"]):
                            if url:
                                try:
                                    ...
                                    cols[i % 3].image(response.content, use_column_width=True)
                                except:
                                    pass
                    else:
...
# ────────────────────────────── Random Pick (with filters) ──────────────────────────────
    ...
    if not restaurants:
        ...
        with col1:
            cuisine_filter = st.multiselect("Cuisine", sorted({r["cuisine"] for r in restaurants}), key="cuisine_filter")
            price_filter = st.multiselect("Price", sorted({r["price"] for r in restaurants}, key=len), key="price_filter")
            type_filter = st.selectbox("Type", ["all", "restaurant", "cocktail_bar"],
                                       format_func=lambda x: {"all":"All", "restaurant":"Restaurants 🍽️", "cocktail_bar":"Bars 🍸"}[x], key="type_filter")
            only_fav = st.checkbox("Only favorites ❤️", key="only_fav")
            visited_filter = st.selectbox("Visited", VISITED_OPTIONS, key="visited_filter")
        with col2:
            location_filter = st.multiselect("Neighborhood", sorted({r["location"] for r in restaurants}), key="location_filter")
