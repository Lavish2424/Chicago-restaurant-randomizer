...
# === TAB SWITCHING LOGIC ===
# If we just added a place successfully, force switch to View All Places once
if "switch_to_view_all" in st.session_state:
    action = "View All Places"
    del st.session_state.switch_to_view_all  # Only do it once
else:
        ...
        for idx, r in enumerate(sorted_places):
            ...
            with st.expander(f"{r['name']}{icon}{fav}{visited} • {r['cuisine']} • {r['price']} • {r['location']}{img_count}{notes_count}",
                ...
                if f"edit_mode_{global_idx}" not in st.session_state:
                    ...
                    # (Your full edit form code here — unchanged)
                    st.subheader(f"Editing: {r['name']}")
                    with st.form(key=f"edit_form_{global_idx}"):
                        # ... (keep all your existing edit fields)
                        pass  # Replace with your original edit form
...
# ────────────────────────────── Random Pick ──────────────────────────────
    ...
    # (Your full Random Pick code — unchanged)
    st.header("🎲 Random Place Picker")
    # ... rest of random picker logic
